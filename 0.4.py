...
class WindowManager:
    ...
    # 原来的 _play_sound 函数将被移除，因为背景音乐是循环播放的
    ...
    def create_window(self):
        ...
        # 注意：这里不再调用 _play_sound，因为背景音乐是独立循环播放的
        # 如果需要新窗口创建时播放音效，需要一个不同的短音效系统
        ...
        def demote_handler(hwnd, extra):
            ...
            try:
                ...
                if "SunAwtWindow" in class_name or "SunAwtFrame" in class_name:
                    # print(f"发现Java窗口，尝试置底: HWND={hwnd}, 类名='{class_name}'") # 调试信息可以暂时关闭
                    # 强制将Java窗口置于所有窗口的底部
                    win32gui.SetWindowPos(hwnd, win32con.HWND_BOTTOM, 0, 0, 0, 0,
                                          win32con.SWP_NOMOVE | win32con.SWP_NOSIZE | win32con.SWP_NOACTIVATE)
                    # print(f"Java窗口 HWND {hwnd} 已置底。")
            except Exception as e:
                # print(f"处理Java窗口 HWND {hwnd} 时出错: {e}") # 避免过多打印
                pass
            return True
        ...
        while self.moving_active:
            ...
            for info in self.windows[:]:  # 遍历副本以安全修改列表
                ...
                if new_x <= 0 or new_x + width >= screen_width:
                    ...
                    # 注意：此处不再调用 _play_sound，因为背景音乐是循环播放的
                ...
                try:
                    # 更新窗口位置并保持在最顶层
                    # 移除 SWP_NOZORDER 意味着每次都尝试重新设置 Z-order
